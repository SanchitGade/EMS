  ...
  try {
    ...
    if (employee.isDeleted) {
      return res.status(403).json({
        error: "You account is deactivated. You cannot apply for leave. ",
      });
    }
...
export const getLeaves = async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Failed to Fetch Leave" });
  }
};
...
export const updateLeaveStatus = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Updated the status", data: leave });
  } catch (error) {
